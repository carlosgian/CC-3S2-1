...
Entonces se muestra un tablero vacío
Y se comienaza el juego
```
...
Dado el usuario a punto de hacer un movimiento
Cuando trate de poner un símbolo O/X en una fila fuera de los límites
Entonces no se pone ningún símbolo en dicha fila.
```
```
AC 1.3 Referencia de columna no válida
Dado el usuario a punto de hacer un movimiento
Cuando trate de poner un símbolo O/X en una columna fuera de los límites
Entonces no se pone ningún símbolo en dicha columna.
```
...
- ¿Cuál es el tipo de datos de las celdas del tablero? 
  Los datos en las celdas del tablero son INT.
 
- ¿Qué valor representa `empty`? 
  El 0.

-  ¿Cómo obtener una celda de tablero dada?
  Con el método getCell de la clase Board.

. ¿Cuál es el tipo de datos de  ' turn de X'?
  El tipo de datos del turno es CHAR.

- ¿Cómo obtener el estado de `turn`? 
  Con el método getTurn de la clase Board.
...
En esta primera parte se quiere comprobar que el tablero se muestre vacío en la consola. Para esto en la parte de @Test simplemente se llama a la función DisplayBoard que dibuja un tablero lleno de 0s(vacío) en la consola.

La etiqueta @Before es lo que se hace antes de correr el test, en este caso, crear una instancia de la clase Board llamada board.
...
Esta parte del código quiere llevar el tablero de juego a la consola.

Para esto se modela a la consola como una clase, la cual solo tiene un método constructor, el cual requiere un objeto del tipo Board(tablero). Es decir, la clase Consola solo se puede instanciar con un objeto del tipo Board ya existente. 

El método displayBoard "dibuja" la cuadrícula(grid) que es un atributo del objeto Board. Para esto hace uso de un for simple y del método Board.getCell que retorna el valor entero en cierta celda de la cuadrícula(grid), que por defecto es 0.
```
...
Dado el jugador X a punto antes de hacer una jugada
Cuando coloque una X en una celda válida
Entonces la celda toma el valor de X
Y se cambia el turno al jugador O
``` 
```
AC 2.2 Un movimiento X ilegal en una celda ocupada
Dado el jugador X antes de hacer una jugada
Cuando  coloque una X en una celda ya ocupada por otro símbolo
Entonces la celda mantiene el valor que tenía
Y no se cambia el turno.
 ...
 Dado el jugador X antes de hacer una jugada
 Cuando  coloque una X en una celda fuera de los límites del tablero
 Entonces se ignora dicho movimiento
 Y no se cambia el turno
```
...
Dado un juego en curso en el turno de la computadora
Cuando la computadora solo le falta un momvimiento para completar un 3 en raya
Entonces la computadora completa el 3 en raya
Y procede a ganar el juego
``` 
```
AC 5.3 Una jugada de bloqueo de la computadora
Dado un juego en curso en el turno de la computadora
Cuando al jugador solo le falta un movimiento para completar un 3 en raya
Entonces la computadora bloquea esa posibilidad, colocando su símbolo sobre la celda correspondiente
Y se procede a cambiar de turno
``` 
``` 
